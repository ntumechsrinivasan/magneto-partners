    ...
    <div className="rv border border-[var(--line)]">
      <div className="grid grid-cols-1 border-b border-[var(--line)] lg:grid-cols-[200px_1fr]">
        ...
        <div className="flex flex-col gap-3.5 p-8">
          ...
          <h2 className="display text-[32px]">Dr Raghavan Gopalan</h2>
          <p className="text-[13.5px] font-light leading-[1.6] text-[var(--mute)]">
      ...
      <div className="grid grid-cols-1 border-b border-[var(--line)] sm:grid-cols-2 lg:grid-cols-4">
        {CREDENTIAL_GRID.map((c, i) => (
          ...
          >
            <b className="display num text-[26px] font-bold text-[var(--nd)]">{c.value}</b>
            <span className="text-[11.5px] font-light leading-[1.5] text-[var(--mute)]">
              {c.label}
            </span>
          </div>
        ))}
      ...
      <div className="flex flex-col gap-8 p-8">
        <div className="flex flex-col gap-[15px] text-[14.5px] font-light leading-[1.75] text-[var(--mute)]">
          {bio.map((p, i) => (
            <p key={i}>{p}</p>
          ))}
        </div>
        ...
        <div>
          ...
          <ul className="flex flex-col">
            {AWARDS.map((a) => (
              ...
              >
                <span className="num font-[family-name:var(--font-jetbrains)] text-[12px] text-[var(--nd)]">
                  {a.year}
                </span>
                <span className="text-[13.5px] font-light leading-[1.5] text-[var(--mute)]">
                  {a.text}
                </span>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="flex flex-col">
            {GOVERNMENT_ROLES.map((r) => (
              <li
                key={r.role}
                className="border-t border-[var(--line)] py-2.5 text-[13px] font-light leading-[1.55] text-[var(--mute)] first:border-t-0"
              >
                {r.role}
              </li>
            ))}
