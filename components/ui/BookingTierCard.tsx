  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        {tier.featuredBadge && (
          <span className="mono bg-[var(--nd)] px-2.5 py-1 text-[12.5px] text-[var(--void)]">
            {tier.featuredBadge}
          </span>
        )}
      </div>
      ...
      <h3 className="text-[20.5px] font-semibold">{tier.name}</h3>

      <ul className="flex flex-col">
        {tier.features.map((f) => (
          <li
            key={f}
            className="grid grid-cols-[20px_1fr] gap-1.5 border-t border-[var(--line)] py-[9px] text-[16px] font-light leading-[1.5] text-[var(--mute)] first:border-t-0 first:pt-0"
          >
      ...
      <button
        ...
        className={`mt-auto inline-flex items-center justify-center gap-2 rounded-[2px] px-[18px] py-[13px] text-[16px] font-semibold transition-colors duration-200 ${
          featured
