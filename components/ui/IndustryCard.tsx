...
export default function IndustryCard({ industry }: { industry: Industry; index?: number }) {
  return (
    <article className="rv group flex flex-col gap-3 border-b border-r border-[var(--line)] px-7 pb-9 pt-8 transition-colors duration-300 hover:bg-[var(--panel)]">
      ...
      <h3 className="display-tight text-[20.5px]">{industry.name}</h3>
      <span className="num font-[family-name:var(--font-jetbrains)] text-[15.5px] tracking-[0.04em] text-[var(--flux)]">
        {industry.cagr}
      </span>
      ...
      <p className="text-[16.5px] font-light leading-[1.65] text-[var(--mute)]">{industry.description}</p>
    </article>
  );
}
